...
Each release is tagged with its version (`:26.7.1`) and `:latest`.
...
Each release pushes both `:<version>` (e.g. `:26.7.1`) and updates `:latest` to point at it. Pick the tag that matches how you want to handle upgrades:

```bash
docker pull ghcr.io/groupdocs-metadata/metadata-net-mcp:26.7.1   # pinned to exact version
...
|---|---|---|
| `:26.7.1` | Locked to that release. No surprise upgrades. | Committed configs, CI, shared team setups. |
...
(
  ...
  sleep 2
    ...
    -v $(pwd)/documents:/data \
    ghcr.io/groupdocs-metadata/metadata-net-mcp:26.7.1 2>/dev/null
```

Expected: two JSON-RPC responses on stdout. The second includes `read_metadata`,
`search_metadata`, `write_metadata`, `remove_metadata`, and `get_document_info` with their
...
services:
  groupdocs-metadata-mcp:
    image: ghcr.io/groupdocs-metadata/metadata-net-mcp:26.7.1
    stdin_open: true
    tty: false
    environment:
...
docker inspect ghcr.io/groupdocs-metadata/metadata-net-mcp:26.7.1 \
